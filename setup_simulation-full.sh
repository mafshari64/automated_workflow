...
# some  parts of the current shell script should to be chnaged in future. search "STBCIF" term in the following.
...
# 0- We are in the parent directory where this setup_simulation.sh script exists.  

# 1- Define the folder name ***ALREADY*** created in the parent directory for each simulation:
simulation_directory="./testPICMI-v2"  # Modify this for each simulation

# 2- the name of the input_file of the simulation (in this case a python script using PICMI standard rules) ***ALREADY*** available in simulation_directory (task 1).
picmi_input_file_name="picmi_inputfile_ionization-v2.py" 

# 3- define codes github path is used for the simulation 
...
SOURCE_DIR="/home/afshar87/afshari/simulation/picInputs/testPICMI-v2/customTemplates/" 
DEST_DIR="/home/afshar87/afshari/simulation/simulation_auto/testPICMI-v2/customTemplates/"
...
# 3.6 Define the output folder where simulation data are dumped (in general in supercomputer)

simulation_output_folder="lwfa-ionization-auto"  # Replace with your specific output folder name

# 3.7 Define the output directory path based on your Python script
output_directory_path="${simulation_directory}/lwfa_ionization_v2"  # lwfa_ionization_v2 is already defined in the picmi python script, i.e. task no.2: picmi_input_file_name="picmi_inputfile_ionization-v2.py" 
...
echo "Starting rsync CUSTOM_TEMPLATE between ${SOURCE_DIR} and ${DEST_DIR}..." 
...
# Run the simulation (in the case of picongpu: tbg command) with the specified configuration file and output folder 
# tbg -f -s -t -c etc/picongpu/N.cfg "${SCRATCH}/${simulation_output_folder}"

# TEMPLATE_PATH="./picongpu/etc/picongpu/hemera-hzdr/fwkt_v100.tpl"
# # Change directory to the folder containing your script and PIConGPU code
# cd "$(dirname "$0")"

# tbg -f -s -t -c etc/picongpu/N.cfg "./picongpu/etc/picongpu/hemera-hzdr/fwkt_v100.tpl" "SCRATCH/${simulation_output_folder}"
# tbg -f -s -t -c etc/picongpu/N.cfg $TEMPLATE_PATH "${SCRATCH}/${simulation_output_folder}"

(cd ${simulation_directory}/${simulation_output_folder} && tbg -f -s -t ./picongpu/etc/picongpu/N.cfg SCRATCH/${simulation_output_folder})
# tbg -f -s -t    etc/picongpu/N.cfg "SCRATCH/${simulation_output_folder}"
